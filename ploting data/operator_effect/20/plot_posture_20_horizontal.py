...
# 读取文件1的数据
x1, y1 = read_data(os.path.join(horizontal, filename_l1))
# 读取文件2的数据
x2, y2 = read_data(os.path.join(horizontal, filename_r1))

# 读取文件3的数据
x3, y3 = read_data(os.path.join(horizontal, filename_c1))
# 读取文件4的数据
x4, y4 = read_data(os.path.join(lateral, filename_c2))
# # 读取文件5的数据
...
# 绘制第一个文件的数据
ax.plot(x1, y1, color='blue', label='left operator')
#
# 绘制第二个文件的数据
ax.plot(x2, y2, color='red', label='right operator')

# 绘制第三个文件的数据
ax.plot(x3, y3, '--', color='blue', label='center operator')
# #
# # 绘制第四个文件的数据
...
plt.savefig("20_posture_horizontal_average.png")
# 显示图表
plt.show()
